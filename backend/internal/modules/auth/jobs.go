...
func (SendPasswordResetEmailArgs) Kind() string { return "auth.send_password_reset_email" }
...
func (w *SendPasswordResetEmailWorker) Work(ctx context.Context, job *river.Job[SendPasswordResetEmailArgs]) error {
	if w.Mailer == nil {
		w.Logger.InfoContext(ctx, "auth: would send password reset email (no SMTP configured — set SMTP_HOST)",
			"job_id", job.ID,
			"user_id", job.Args.UserID,
			"email", job.Args.Email,
		)
	...
	if err := w.Mailer.SendPasswordResetEmail(ctx, job.Args.Email, job.Args.Username, job.Args.Token); err != nil {
		w.Logger.ErrorContext(ctx, "auth: sending password reset email failed",
			"job_id", job.ID,
			"user_id", job.Args.UserID,
			"error", err,
		)
	...
	w.Logger.InfoContext(ctx, "auth: sent password reset email",
		"job_id", job.ID,
		"user_id", job.Args.UserID,
		"email", job.Args.Email,
	)
...
func RegisterWorkers(logger *slog.Logger, mailer Mailer) jobs.WorkerRegistrar {
	return func(workers *river.Workers) {
		...
		river.AddWorker(workers, &SendPasswordResetEmailWorker{Logger: logger, Mailer: mailer})
	}
}
